...
const SLCollect = (() => {
  ...
  const RAIL = /\b(recommend\w*|suggest\w*|wish\s*list|watch\s*list|because you watched|more like|similar|also (?:bought|liked|watched)|trending|popular|new releases?|coming soon|continue watching|top picks|featured|sponsored|you might)\b/i;
  function railRoots(){
    const roots = [];
  ...
  function ownedLinks(selector){
    ...
    // Safety valves: never empty the library, and never trust a heading
    // that would sweep away half of it (a page-level "Browse" banner).
    return kept.length && kept.length >= links.length * 0.5 ? kept : links;
  }
