...
let posters = LS.get('posters', {});       // id -> url ('' = not found)
  ...
  while (pQueue.length){
    ...
    try {
      ...
      if (key){
        ...
        const r = await fetch(`https://api.themoviedb.org/3/search/${kind}?api_key=${key}&query=${encodeURIComponent(e.title)}${e.year?`&year=${e.year}`:''}`);
        ...
        const r = await fetch(`https://itunes.apple.com/search?term=${encodeURIComponent(e.title)}&media=${e.type==='tv'?'tvShow':'movie'}&entity=${kind}&limit=5`);
        const j = await r.json();
        let hits = j.results||[];
        const tnorm = normq(e.title);
      ...
      if (!url) url = SLMerge.storePoster(e); // artwork from the store itself
      if (!url){
        // Keyless fallback: Wikipedia lead images. Film/TV posters are
        // fair-use, so pilicense=any is required; origin=* enables CORS
        // from any origin, including file:// and chrome-extension:.
        const q = encodeURIComponent(`${e.title} ${e.type==='tv'?'TV series':'film'}`);
        const r = await fetch(`https://en.wikipedia.org/w/api.php?action=query&generator=search&gsrsearch=${q}&gsrlimit=1&prop=pageimages&piprop=thumbnail&pithumbsize=342&pilicense=any&format=json&origin=*`);
        const j = await r.json();
...
SLCollect.google().forEach(it=>{if(seen.has(it.key))return;seen.add(it.key);out.push({title:it.title,url:it.url,tv:it.tv})});
...
const out=[],seen=new Set();
SLCollect.amazon(location.pathname.includes('/tv')).forEach(it=>{if(seen.has(it.key))return;seen.add(it.key);out.push({title:it.title,url:it.url,tv:it.tv})});
...
const out=[],seen=new Set();
SLCollect.fandango().forEach(it=>{if(seen.has(it.key))return;seen.add(it.key);out.push({title:it.title,url:it.url,tv:it.tv})});
